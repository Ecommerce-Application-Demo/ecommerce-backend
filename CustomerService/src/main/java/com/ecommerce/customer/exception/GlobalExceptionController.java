...
public class GlobalExceptionController {
	...
	public ResponseEntity<ErrorResponse> credentialException(BadCredentialsException ex) {
		...
		ErrorResponse response = new ErrorResponse(ErrorCode.INVALID_CREDENTIAL.getErrorMessage(),ex.getMessage(),ErrorCode.INVALID_CREDENTIAL.getErrorCode());
	...
	public ResponseEntity<ErrorResponse> messagingException(MessagingException ex) {
		ErrorResponse response = new ErrorResponse(ex.getMessage(),null,100);
		return new ResponseEntity<>(response, HttpStatus.INTERNAL_SERVER_ERROR);
	}
